...
class gaussian_kde(object):
    ...
    """
    def __init__(self, dataset, weights, bw_method=None):
    ...
    def integrate_gaussian(self, mean, cov):
        ...
        result = sum(self.weights*exp(-energies), axis=0) / sqrt(linalg.det(2 * pi *
                                                        sum_cov)) / self.n
    ...
    def integrate_kde(self, other):
        ...
        for i in range(small.n):
            ...
            energies = sum(diff * tdiff, axis=0) / 2.0
            result += sum(self.weights*exp(-energies), axis=0)
